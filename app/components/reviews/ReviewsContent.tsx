...
const ReviewsData = ({ reviews, legends }: any) => {
    ...
    const nextReview = () => {
        setCurrentIndex((prevIndex) => {
            if (visibleCount === 1 && prevIndex < reviewsUpdate.length - 1) {
                return prevIndex + 1;
            }
            return prevIndex;
        });
    ...
    const handleShowMore = () => {
        if (visibleCount + 5 <= reviewsUpdate.length) {
            setVisibleCount(visibleCount + 5); // Increment by 5 each click, up to the total length.
        } else if (visibleCount < reviewsUpdate.length) {
            setVisibleCount(reviewsUpdate.length); // Show all if adding 5 exceeds the total length.
        } else {
    ...
    const displayedReviews = visibleCount === 1 ? [reviewsUpdate[currentIndex]] : reviewsUpdate.slice(0, visibleCount);
            ...
            <div className="  text-black p-4  2xl:min-h-[60vh] flex justify-center flex-wrap items-center">
                <div className="w-full flex justify-end mb-2">
                    <span className="text-sm bg-primary py-1 px-3 rounded-full">
                        {visibleCount === 1 ? `Review ${currentIndex + 1} of ${reviewsUpdate.length}` : `Showing ${visibleCount} of ${reviewsUpdate.length}`}
                    </span>
                </div>
                ...
                <div className="w-auto justify-center items-center flex">
                    <div className="flex w-full">
                        ...
                        {visibleCount === 1 && (
                            <button onClick={nextReview} disabled={currentIndex >= reviewsUpdate.length - 1} className={`${currentIndex >= reviewsUpdate.length - 1 ? 'text-gray-500 ml-4' : 'text-primary ml-4'}`}>
                ...
                <div className="flex w-full justify-center">
                    <button className="mt-4 w-auto px-6 py-2 border border-orange-500 bg-primary text-white rounded-full hover:bg-orange-500 hover:text-white transition-colors"
                        ...
                        {visibleCount < reviewsUpdate.length ? 'SHOW MORE' : 'SHOW LESS'}
                    </button>
                </div>
